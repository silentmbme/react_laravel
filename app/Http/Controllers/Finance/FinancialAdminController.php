...
class FinancialAdminController extends Controller {
 ...
 public function taxReport(Request $request) {
  ...
  $f=$request->validate(['from'=>['nullable','date'],'to'=>['nullable','date','after_or_equal:from']]);
  $orders=Order::query()->where('status','paid')->when($f['from']??null,fn($q,$v)=>$q->whereDate('paid_at','>=',$v))->when($f['to']??null,fn($q,$v)=>$q->whereDate('paid_at','<=',$v))->get();
  $lines=$orders->groupBy(fn($order)=>($order->tax_snapshot['tax_code']??'UNSPECIFIED').'|'.($order->tax_snapshot['tax_name']??'Tax'))->map(function($group,$key){
   [$code,$name]=explode('|',$key,2); return ['tax_code'=>$code,'tax_name'=>$name,'rate'=>$group->first()->tax_snapshot['tax_rate']??null,'orders'=>$group->count(),'tax_collected'=>(float)$group->sum('tax'),'currency'=>$group->first()->currency];
  })->values();
  return response()->json(['from'=>$f['from']??null,'to'=>$f['to']??null,'lines'=>$lines,'total_tax_collected'=>(float)$orders->sum('tax'),'notice'=>'Use this audit report for remittance preparation. Filing or payment to a tax authority requires an authorized accountant or connected tax provider.']);
